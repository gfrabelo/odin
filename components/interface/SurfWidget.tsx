...
function MiniCompass({ deg, label, colorClass = "text-[var(--odin-accent)]" }: { deg: number; label: string; colorClass?: string }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1.5">
      ...
      <div className="relative size-14 rounded-full border border-white/5 bg-neutral-950/20 flex items-center justify-center">
        {/* Marcações */}
        ...
        >
          <div className="relative w-full h-full flex items-center justify-center">
            ...
            {/* Haste central */}
            <div className="w-[1px] h-6 bg-neutral-800 absolute" />
          </div>
        </div>
      </div>
    </div>
...
function WaveSparkline({ waveHours }: { waveHours: Array<{ wave_height: number }> }) {
  ...
  return (
    <div className="flex flex-col gap-1 w-full bg-neutral-950/10 rounded-lg p-2 border border-white/5">
      <div className="flex justify-between text-[8px] uppercase tracking-wider text-neutral-500 font-mono">
    ...
    <div className="flex flex-col gap-4 font-mono text-xs">
      ...
      <div className="flex items-center justify-between">
        <div className="flex flex-col gap-0.5">
          <span className="text-[10px] uppercase tracking-wider text-neutral-500">Localização</span>
          <span className="text-neutral-100 font-bold text-[11px] tracking-wide">{data.spot}</span>
        </div>
        <div className="flex flex-col items-end gap-0.5">
          <span className="text-[10px] uppercase tracking-wider text-neutral-500">Qualidade</span>
          <span className={cn(
            "font-bold uppercase tracking-wider text-[11px] px-2 py-0.5 rounded border transition-all duration-300",
